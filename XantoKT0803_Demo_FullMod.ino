...
void printUsage() {
  ...
  Serial.println(" -Sterio/Mono mode change (default: stereo): S");
  Serial.println(" -Auto Volume Control (default:off): A");
  Serial.println(" -Pre-emphasis Time-Constant Set (50/75) (default:75): T");
  //
...
void loop() {
  if (Serial.available()) {
    ...
    if (cmd == 'W') {
      ...
      //fm.write(register_address, value)
      
      Serial.println("Auto volume control changed");

      uint8_t value = fm.read(0x04);
      
      fm.write(0x04, value ^ 0x80);
      printErrorIfExists();
      
    } else if (cmd == 'S') {
      
      Serial.println("Stereo mode changed");

      uint8_t value = fm.read(0x04);
      
      fm.write(0x04, value ^ 0x40);
      printErrorIfExists();
      
    } else if (cmd == 'T') {
      
      Serial.println("Pre-emphasis time constant changed");

      uint8_t value = fm.read(0x02);
      
      fm.write(0x02, value ^ 0x01);
      printErrorIfExists();
